...
import { AlertCircle, CheckCircle2, Copy, Download, FileText, History, Image as ImageIcon, Sparkles, Trash2, X, Zap } from 'lucide-react';
import './styles.css';

const formats=[['sales_letter','✉️','セールスレター'],['manga','🎨','漫画シナリオ'],['storyboard','🎬','動画絵コンテ'],['flyer','📄','チラシ構成案'],['pamphlet','📑','パンフレット'],['image','🖼️','画像生成']];
...
function App(){
  ...
  const prompt=useMemo(()=>{
    if(outputFormat==='image'){
      return `以下の情報をもとに、${imageStyle}の画像を1枚生成してください。用途に合う構図・光・背景・視線誘導まで設計してください。画像内の文字は、ユーザーが明示した場合だけ入れてください。\n\n${material}\n\n追加の画像指示：${imagePrompt||'（なし）'}`;
    }
  ...
  },[outputFormat,imageStyle,imagePrompt,material,mode,basicType,bookName]);

  const showToast=m=>{setToast(m);setTimeout(()=>setToast(''),2500)};
  const saveHistory=(text,extra={})=>{const item={id:String(Date.now()),date:new Date().toLocaleString('ja-JP',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}),title:`${formatLabel(outputFormat)} (${modeLabel(mode)})`,state:{formData,mode,basicType,bookName,outputFormat,resultAi:text,prompt,...extra}};const next=[item,...history].slice(0,50);setHistory(next);localStorage.setItem('contentMakerHistory',JSON.stringify(next));};
  ...
  const callImage=async()=>{const r=await fetch('/api/generate-image',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({prompt,aspectRatio:imageRatio,imageSize:'1K'})});const d=await r.json();if(!r.ok)throw new Error(d.error||'画像生成に失敗しました');if(!d.image)throw new Error('画像データを取得できませんでした');return `data:${d.mimeType||'image/png'};base64,${d.image}`;};
  const generate=async()=>{setLoading(true);setError('');setActiveTab('ai');try{if(outputFormat==='image'){const img=await callImage();setImageResult(img);setResultAi('');saveHistory('',{imageResult:img,imagePrompt,imageRatio,imageStyle});showToast('🖼️ 画像を生成しました');}else{const t=await callAI(prompt);setResultAi(t);setImageResult('');saveHistory(t);showToast('✨ 生成完了！履歴に保存しました');}}catch(e){setError(e.message)}finally{setLoading(false)}};
  ...
  const loadItem=i=>{setFormData(i.state.formData);setMode(i.state.mode);setBasicType(i.state.basicType);setBookName(i.state.bookName);setOutputFormat(i.state.outputFormat);setResultAi(i.state.resultAi||'');setImageResult(i.state.imageResult||'');setImagePrompt(i.state.imagePrompt||'');setImageRatio(i.state.imageRatio||'4:5');setImageStyle(i.state.imageStyle||'広告・アイキャッチ');setActiveTab('ai');setHistoryOpen(false)};
  ...
  return <div className="app">
    ...
    <main className="grid">
      ...
      <div className="right">
        <section className="card"><div className="card-title"><span className="gold">2</span><b>用途と設定</b></div><div className="pad"><p className="eyebrow">何を作成しますか？</p><div className="format-grid">{formats.map(([id,e,l])=><button key={id} className={outputFormat===id?'selected':''} onClick={()=>setOutputFormat(id)}><i>{e}</i>{l}</button>)}</div>{outputFormat==='image'?<div className="image-settings"><p className="eyebrow">画像の追加指示</p><textarea value={imagePrompt} onChange={e=>setImagePrompt(e.target.value)} placeholder="例：白背景、手描き線画、文字なし、ピンク・黄色・ミント。人物はやさしい表情。"/><div className="image-options"><label>テイスト<select value={imageStyle} onChange={e=>setImageStyle(e.target.value)}>{imageStyles.map(v=><option key={v}>{v}</option>)}</select></label><label>比率<select value={imageRatio} onChange={e=>setImageRatio(e.target.value)}>{ratios.map(v=><option key={v}>{v}</option>)}</select></label></div></div>:<><p className="eyebrow">アプローチ手法</p><div className="mode-grid">{modes.map(([id,l,d])=><button key={id} className={mode===id?'selected violet':''} onClick={()=>setMode(id)}><b>{l}</b><small>{d}</small></button>)}</div><div className="mode-detail">{mode==='basic'&&<select value={basicType} onChange={e=>setBasicType(e.target.value)}><option value="A">⚡ 心理直撃型 — Cashvertising</option><option value="B">📖 ストーリー型 — StoryBrand</option><option value="C">✨ 差別化型 — Fascinate</option><option value="D">🚀 発想ショートカット型 — The Cheat Code</option></select>}{mode==='legend'&&<p>歴代のマーケティング巨匠の原理を組み合わせて構成します。</p>}{mode==='book'&&<input value={bookName} onChange={e=>setBookName(e.target.value)} placeholder="参考にしたい書籍名・著者名"/>}{mode==='infinite'&&<p>ターゲットに合う5つのアプローチを選定し、5パターン生成します。</p>}</div></>}
        <button className="generate" onClick={generate} disabled={loading}><Zap size={20}/>{loading?(outputFormat==='image'?'画像を生成中…':'AIが執筆中…'):`${formatLabel(outputFormat)} を生成する`}</button></div></section>
        <section className="card output"><div className="tabs"><button className={activeTab==='ai'?'on':''} onClick={()=>setActiveTab('ai')}>✨ AI出力結果</button><button className={activeTab==='prompt'?'on':''} onClick={()=>setActiveTab('prompt')}>📝 プロンプト確認</button></div><div className="output-body">{activeTab==='prompt'?<><div className="result-head"><span>生成に使う指示</span><button className="copy" onClick={()=>copy(prompt)}><Copy size={13}/>コピー</button></div><textarea className="prompt" readOnly value={prompt}/></>:outputFormat==='image'?(imageResult?<div className="image-result"><div className="result-head"><span><CheckCircle2 size={15}/>画像完成</span><a className="copy download" href={imageResult} download="generated-image.png"><Download size={13}/>保存</a></div><img src={imageResult} alt="AI生成画像"/><p>Gemini 3.1 Flash Imageで生成</p></div>:<div className="empty"><ImageIcon size={32}/><p>素材と画像の指示を入力して<br/>「画像生成」を押してください</p></div>):resultAi?<><div className="result-head"><span><CheckCircle2 size={15}/>完成</span><button className="copy" onClick={()=>copy(resultAi)}><Copy size={13}/>コピー</button></div><pre className="result">{resultAi}</pre><div className="refine"><p>✨ 仕上げの推敲</p><div>{refineOptions.map(([k,l])=><button disabled={loading} key={k} onClick={()=>refine(k)}>{l}</button>)}</div></div></>:<div className="empty"><ImageIcon size={32}/><p>素材を入力して<br/>生成ボタンを押してください</p></div>}</div></section>
      </div>
    </main>
