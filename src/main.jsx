...
const formats=[['sales_letter','✉️','セールスレター'],['manga','🎨','漫画シナリオ'],['storyboard','🎬','動画絵コンテ'],['flyer','📄','チラシ構成案'],['pamphlet','📑','パンフレット'],['image','🖼️','画像プロンプト']];
...
function App(){
  ...
  },[outputFormat,imageStyle,imageRatio,imagePrompt,material,mode,basicType,bookName]);

  const showToast=m=>{setToast(m);setTimeout(()=>setToast(''),2500)};
  const saveHistory=text=>{const item={id:String(Date.now()),date:new Date().toLocaleString('ja-JP',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}),title:`${formatLabel(outputFormat)} (${modeLabel(mode)})`,state:{formData,mode,basicType,bookName,outputFormat,resultAi:text,prompt,imagePrompt,imageRatio,imageStyle}};const next=[item,...history].slice(0,50);setHistory(next);localStorage.setItem('contentMakerHistory',JSON.stringify(next));};
  ...
  const generate=async()=>{setLoading(true);setError('');setActiveTab('ai');try{const t=await callAI(prompt);setResultAi(t);saveHistory(t);showToast(outputFormat==='image'?'📝 画像プロンプトを作成しました':'✨ 生成完了！履歴に保存しました');}catch(e){setError(e.message)}finally{setLoading(false)}};
  ...
  const loadItem=i=>{setFormData(i.state.formData);setMode(i.state.mode);setBasicType(i.state.basicType);setBookName(i.state.bookName);setOutputFormat(i.state.outputFormat);setResultAi(i.state.resultAi||'');setImagePrompt(i.state.imagePrompt||'');setImageRatio(i.state.imageRatio||'4:5');setImageStyle(i.state.imageStyle||'広告・アイキャッチ');setActiveTab('ai');setHistoryOpen(false)};
